...
function C = Cost(X, ~)
     C =  1/10*sum(X.^2,2).*cos(1.6*sum(X, 2)).^2.*exp(-.05*(X(:,1).^2 + X(:,2).^2));
end
function V = Value(positions, K, exit, adjacency)
    ...
    while size(Tentative) ~= 0 
         ...
         to_delete = [];
         for i = 1:size(pick)
             if forbidden_dynamics(pick(i), current_node, positions)
...
function fail = forbidden_dynamics(origin, target, positions)
    direction = positions(origin,:) - positions(target, :);
    if direction(2)/abs(direction(1))> .51 + .54*cos(2*sum(positions(origin,:)))^2
        fail = true;    
    else 
        fail = false; 
    end
